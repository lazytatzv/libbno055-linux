...
TEST_F(ROS2NodeTest, TestPerfPublisherNodeMock) {
    rclcpp::NodeOptions options;
    options.append_parameter_override("device", "/dev/i2c-mock");
    options.append_parameter_override("address", 0x28);
    options.append_parameter_override("publish_rate", 50.0);

    // Create high-perf node (should initialize BNO055 in mock mode)
    auto node = std::make_shared<bno055_ros2::BNO055PerfPublisherNode>(options);
