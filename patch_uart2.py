...
# 9. Modify readLen inside the thread-safe readLen
readLen_search = """#ifdef __linux__
            uint8_t reg_buf[1] = {reg};
...
readLen_replace = """#ifdef __linux__
            if (conn_type_ == ConnectionType::UART) {
                ...
                if (::write(i2c_fd, buf, 4) == 4) {
                    ...
                    if (uart_read_exact(resp, 2)) {
                        if (resp[0] == 0xBB && resp[1] == len) {
                            ...
                            // BUS_OVER_RUN_ERROR, let it retry
...
text = text.replace(readLen_search, readLen_replace)

# 10. Add BNO055 UART constructor
bno_ctor_search = "BNO055::BNO055(uint8_t i2c_address, std::string_view i2c_device)"
bno_ctor_replace = """BNO055::BNO055(const UARTConfig& uart_config) : impl_(std::make_unique<Impl>(uart_config)) {}

BNO055::BNO055(uint8_t i2c_address, std::string_view i2c_device)"""
text = text.replace(bno_ctor_search, bno_ctor_replace)
